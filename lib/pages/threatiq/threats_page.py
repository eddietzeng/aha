...
class ThreatsPage(ThreatiqPage):
    """
    ThreatIQ -> Threats
    This is Threats Page class which inherits from ThreatiqPage
    Threats implementations are defined here
    """

    def click_tab(self):
        """
        This method is to click Threats tab
        """
        try:
            self.click_element(self.locator.tabThreats)
            self.wait_element_visible(self.locator.btnApply, timeout=300)
        except RuntimeError as e:
            raise Exception("Failed to click Threats tab: %s" % e)
