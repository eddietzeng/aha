...
class BasePage:
    ...
    IMG_DIR = os.path.join(os.getcwd(), "avxt/screenshot")
    ...
    def input_text(self, loc, value, timeout=10, frequency=0.5):
        ...
        @param frequency: default is 0.5 secs
        """
        self.wait_element_present(loc, timeout, frequency)
        ele = self.get_element(loc)
        try:
            ele.send_keys(value)
    ...
    def skip_to_free_trial(self):
        btnKeepFree = (By.XPATH, "//*[@id='__next']/div[1]/div/div[1]/button")
        btnSkip = (By.XPATH, "//button[text()='Skip']")
        if self.is_element_visible(btnKeepFree, timeout=1):
            self.click_element(btnKeepFree)
            if self.is_element_visible(btnSkip, timeout=1):
