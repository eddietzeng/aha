...
def create_mailslurp_inbox(api_key, logger):
    ...
    try:
        ...
        logger.error("An exception occurred while creating the MailSlurp inbox: %s\n" % e)
        return None
...
def wait_for_latest_email(api_key, inbox, logger):
    ...
    try:
        ...
        logger.error("An exception occurred while waiting for the latest email: %s\n" % e)
        return None
